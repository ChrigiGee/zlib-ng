...
#define XOR_INITIAL128(where)   if (crc != 0) { crc = 0; where = _mm_xor_si128(where, xmm_initial); }
...
static size_t fold_16(__m128i *xmm_crc0, __m128i *xmm_crc1, __m128i *xmm_crc2, __m128i *xmm_crc3, uint8_t *dst,
    const uint8_t *src, size_t len, __m128i crc, const int COPY) {
...
#ifdef X86_VPCLMULQDQ
    if (len >= 256) {
        size_t n = fold_16(&xmm_crc0, &xmm_crc1, &xmm_crc2, &xmm_crc3, dst, src, len, xmm_initial, COPY);
        len -= n;
        src += n;
        if (COPY) {
            dst += n;
